...
float acumuladorVentas(char fileSell[], float acumulador);
...
int main()
{
    ...
    do
    {
...

        {
            ...
            float agregarAventas = acumuladorVentas(fileSell, registroVentas);
            printf("\nTotal acumulado $%0.2f: ", agregarAventas);
        }
...
producto checkId(char fileProducts[], producto arr[], int pos)
{
    ...
    if(check!=NULL)
    {
        while(!feof(check))
        {
            ...
            {
//                fseek(check, (-1)*sizeof(producto), SEEK_CUR);
                retornoProd = checkeo;
            }
        }
        fclose(check);
...
float acumuladorVentas(char fileSell[], float acumulador)
{
    ...
    ventasAcumular agregar;
    float aux = 0.0;
    ventas = fopen(fileSell, "ab");

    if (ventas != NULL)
    {
        fread(&agregar, sizeof(ventasAcumular) , 1, ventas);

        agregar.acumulado += acumulador;

        fseek(ventas, (-1)*sizeof(ventasAcumular), SEEK_CUR);

        fwrite(&agregar, sizeof(ventasAcumular), 1, ventas);

        aux = agregar.acumulado;
